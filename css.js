...
module.exports = {
  ...
  rules: {
    ...
    block: $ => seq('{',
      repeat($._block_item),
      optional(alias($.last_declaration, $.css_declaration)),
      '}'
    ),
    ...
    class_selector: $ => prec(1, seq(
      optional($._selector),
      '.',
      alias($.css_identifier, $.class_name),
    )),
    ...
    id_selector: $ => seq(
      optional($._selector),
      '#',
      alias($.css_identifier, $.id_name)
    ),

    attribute_selector: $ => seq(
      optional($._selector),
      '[',
      alias($.css_identifier, $.attribute_name),
      optional(seq(
        choice('=', '~=', '^=', '|=', '*=', '$='),
        $._value
      )),
    ...
    css_declaration: $ => seq(
      alias($.css_identifier, $.property_name),
      ':',
      $._value,
      repeat(seq(
        optional(','),
