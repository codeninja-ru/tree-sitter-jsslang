...
#	wget https://raw.githubusercontent.com/tree-sitter/tree-sitter-javascript/master/src/tree_sitter/parser.h -O ./src/tree_sitter/parser.h
	...
	find /tmp/javascript/highlight -type f -exec bash -c 'mv $$0 $${0/.js/.jss}' {} \;
	...
	mkdir /tmp/queries
	wget https://codeload.github.com/tree-sitter/tree-sitter-javascript/tar.gz/master -O - | tar -xzv -C /tmp/queries --strip=2 "tree-sitter-javascript-master/queries"
	cp /tmp/queries/* ./queries
#	rm -rf /tmp/javascript
#	rm -rf /tmp/css
